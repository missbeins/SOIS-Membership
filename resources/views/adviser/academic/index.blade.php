                            ...
                            @if ($academic_memberships->isNotEmpty())
                                @foreach ($academic_memberships as $membership)
                                    <tr>
                                        ...
                                        @endif
                                        <td>
                                            <a href="{{ route('membership.adviser.academic.memberships-members', $membership->academic_membership_id) }}" type="button" class="btn btn-info"><i class="fas fa-eye me-2"></i>Members</a>

                                            <a href="{{ route('membership.adviser.academic.memberships-details', $membership->academic_membership_id) }}" type="button" class="btn btn-info"><i class="fas fa-eye me-2"></i>Details</a>
                                            {{-- <a href="{{ route('membership.adviser.generateAcadMembershipPDF') }}" class="btn btn-warning" onclick="event.preventDefault();
                                            document.getElementById('pdf-form').submit();"><i class="fas fa-file-pdf me-2"></i>Generate PDF</a>
                                         --}}<a href="{{ route('membership.adviser.academic.generateMembershipPDF', $membership->academic_membership_id) }}" class="btn btn-warning"><i class="fas fa-file-pdf me-2"></i>Generate PDF</a>

                                            {{-- <form id="pdf-form" action="{{ route('membership.adviser.generateAcadMembershipPDF') }}" method="POST" class="d-none">
                                                @csrf
                                                <input type="hidden" name="membership_id" value="{{ $membership->academic_membership_id }}">
                                            </form> --}}
                                            <!-- Button trigger modal -->
                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#report{{ $membership->academic_membership_id }}">
                                                <i class="fas fa-file-pdf me-2"></i>Generate PDF by Year Level
                                            </button>
                                            @include('adviser.academic.includes.generate-pdf')
                                        </td>
                                    </tr>
                                @endforeach
                        @else

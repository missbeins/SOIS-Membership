...
<div class="card">
    ...
    <div class="card-body">
        <table class="table table-striped" id="acadsorgs">
            <thead>
                <th>Organization</th>
                <th>Start Date</th>
                <th>End Date</th>
            ...
            <tbody>
                ...
                @foreach ($organizations as $organization)
                    <tr>
                        ...
                        @if ($organization->am_status == 'Ended' || $organization->am_status == 'ended')
                            <td><span class="badge bg-danger">{{ $organization->am_status }}</span></td>
                        @else
                            <td><span class="badge bg-success">{{ $organization->am_status }}</span></td>
                        @endif
...
@section('scripts')
    <script type="module">
        ...
        window.addEventListener('DOMContentLoaded', event => {
            const dataTable = new simpleDatatables.DataTable("#acadsorgs", {
                perPage: 5,
                searchable: true,
                labels: {
